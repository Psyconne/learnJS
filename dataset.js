...
//Demonstrate forEach, filter, map and reduce standard array methods

let newScripts = SCRIPTS.filter(s => s.year > 1980);
let ranges = newScripts.map(s => s.ranges);
console.log(ranges);
let names = newScripts.map(s => s.names);
let repeat = (n, action) => { for (let i=0; i<n; i++) action(i);}; 

ranges.forEach(l => {
    repeat(l.length, n => {
	console.log(l[n], 'Range Number:', n);
    });
});
